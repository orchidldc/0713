...
$g_t$ 和 $h_t$ 都是可交易或不可交易的因子

> [!TIP|label:提示]
> Tradable factors：可由可交易资产组成的因子，例如 FF3 中的 SMB，HML<br>
> Nontradable factors：宏观市场层面的因子，例如 inflation

核心假设：真实的资产定价模型是近似低维的——稀疏性

<div style="border: 2px solid #4bad85; padding: 1px; width: 150px; text-align: center; color: #4bad85">

<font size=5><b>实例：</b></font>
</div>

Heston & Sadka (2008) 提出了季节因子(seasonality factor)，相较于 FF3 模型存在显著的 $\alpha$ (t-static = 2.06)。似乎 seasonality 是一个相对于 FF3 模型有超额收益的因子：

$$
seasonality = \alpha + \beta_1\big(E(R_M)-R_F\big) + \beta_2\big(E(SMB)\big) + \beta_3\big(E(HML)\big)
$$

然而，季节因子和动量因子高度相关 (其与 Carhart 动量因子的相关性为0.63)，如果采用包含动量因子的基准模型进行评估，则alpha 会减小且在统计上不显著 (t-static = -0.87)：

$$
seasonality = \alpha + \beta_1\big(E(R_M)-R_F\big) + \beta_2\big(E(SMB)\big) + \beta_3\big(E(HML)\big) + \beta_4\big(E(UMD)\big)
$$

这个例子说明在评估新因子的有效性时，**基准模型的选择至关重要**

> <p id="p1">&nbsp;&nbsp; 📘: Most papers in the literature that aim to produce new factors choose the benchmark model somewhat <strong>arbitrarily</strong>, subject to potential <strong>data-mining bias</strong>. Our procedure systematically constructs <mark>the best low-dimensional benchmark</mark> to evaluate new factors using the entire factor zoo.</p>
...
- [Feng G, Giglio S, Xiu D. Taming the factor zoo: A test of new factors[J]. The Journal of Finance, 2020, 75(3): 1327-1370.](https://doi.org/10.1111/jofi.12883)